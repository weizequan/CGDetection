...
import networks
import shutil
from IMGSM import IMGSM
from misc_functions import pil_loader, preprocess_image
...
dst_dir = os.path.join(data_root, 'model_centric')
    ...
    for itr in np.arange(1, 21):
        ...
        if len(nature_error_itr_local) > 0:
            ...
            if adv_model_num < 1:
                ...
                checkpoint = torch.load(LOG_DIR + '/checkpoint_pn_' + str(adv_model_idx) + '.pth')
                model.load_state_dict(checkpoint['state_dict'])  # using model with locally largest NI val error rate to update the weights
...
# Construct negative samples using IMGSM
def construct_negative_samples(model, itr):
    assert(itr > 0)
    ...
    for line in image_name:
        img_path = os.path.join(img_des_dir, line)
        img = pil_loader(img_path)
        img_color = preprocess_image(img).cuda()
        
        # Forward pass
        with torch.no_grad():
            output = model(img_color)

        init_pred = output.max(1, keepdim=True)[1] # get the index of the max log-probability

        if init_pred.cpu().item() == target_class:
            continue

        prob_threshold = [0.5, 0.3]
        negative_samples, itrTmp = IMGSM(model, img, target_class, alpha, itr_max, prob_threshold, mask, grad_type)
        
        if negative_samples.width > 233 or negative_samples.height > 233:
            continue
        
        filename, file_extension = os.path.splitext(line)
        new_image_name = filename + '-cgadvsam-' + str(itr) +'.png' 
        negative_samples.save(os.path.join(img_des_dir, new_image_name))
        pn_num += 1
        total_itr += itrTmp
        max_itr = max(max_itr, itrTmp)
    print('The number of samples: ', pn_num)
    print('Average itrations for samples generation: ', total_itr/pn_num)
